...
type Position =
  ({left: CSSVal, width: CSSVal} | {right: CSSVal, width: CSSVal} | {left: CSSVal, right: CSSVal}) &
  ({top: CSSVal, height: CSSVal} | {bottom: CSSVal, height: CSSVal} | {top: CSSVal, bottom: CSSVal})

function update_css(style: CSSStyleDeclaration, key: string, val: number | string | null | undefined): void {
  if (val != null) {
...
export class DialogView extends UIElementView {
  ...
  override render(): void {
    ...
    const buttons_el = div({class: dialogs.buttons})

    this.shadow_el.append(header_el)
    this.shadow_el.append(content_el)
    this.shadow_el.append(buttons_el)
    ...
    const key_press = (event: KeyboardEvent) => {
      if (event.key as Keys == "Escape") {
        ...
        position(this.el, {left, top, width, height})
        cancel()
      }
    }
  ...
  protected _saved_bbox: BBox | null = null

  protected _collapsed: boolean = false
  collapse(): void {
    if (!this._collapsed) {
      if (this._saved_bbox == null) {
        this._saved_bbox = this.bbox.clone()
      }
      const {left, top, width} = this._saved_bbox
      position(this.el, {left, top, width, height: "max-content"})
      this._minimize_el.title = "Restore"
    } else {
      assert(this._saved_bbox != null)
      position(this.el, this._saved_bbox)
      this._minimize_el.title = "Collapse"
      this._saved_bbox = null
    }
  ...
  minimize(): void {
    if (!this._minimized) {
      ...
      position(this.el, {left: 0, bottom: 0, width: "max-content", height: "max-content"})
      this._minimize_el.title = "Restore"
    } else {
      assert(this._saved_bbox != null)
      position(this.el, this._saved_bbox)
  ...
  maximize(): void {
    if (!this._maximized) {
      ...
      position(this.el, {left: 0, top: 0, width: "100%", height: "100%"})
